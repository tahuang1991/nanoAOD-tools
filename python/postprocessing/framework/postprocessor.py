...
class PostProcessor :
    def __init__(self,outputDir,inputFiles,cut=None,branchsel=None,modules=[],compression="LZMA:9",friend=False,postfix=None,
		 jsonInput=None,noOut=False,justcount=False,provenance=False,haddFileName=None,fwkJobReport=False,histFileName=None,histDirName=None, outputbranchsel=None, outtreeName="Friends"):
	...
	for fname in self.inputFiles:
	    ...
	    # prepare output file
            if not self.noOut:
                ...
                # prepare output tree
                if self.friend:
                    outTree = FriendOutput(inFile, inTree, outFile, self.outtreeName)
                else:
                    outTree = FullOutput(
                        inFile,
                        inTree,
